#!/bin/bash
# Single-GPU CLM baseline (master branch, original code).
# Usage:
#   sbatch single_gpu.sh a40      # 1x A40
#   sbatch single_gpu.sh a100     # 1x A100
...
case "${GPU_CHOICE}" in
    ...
    *)    echo "Usage: sbatch single_gpu.sh {a40|nvidia_a40|a100|nvidia_a100}"; exit 1 ;;
esac
...
echo "=== Single-GPU CLM Baseline (master) ==="
echo "GPU: ${GPU_CHOICE}"
...
python train.py \
    ...
    --bsz 8 \
    --eval \
    -m "output/rubble_${GPU_TAG}_single_clm"