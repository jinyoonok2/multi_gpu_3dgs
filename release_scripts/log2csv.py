...
def extract_final_metrics_from_log(log_file):
    ...
    for line in reversed(lines):
        ...
        if metrics["total_time_s"] is None and "end2end total_time:" in line:
            ...
            try:
                metrics["total_time_s"] = float(
                    line.split("end2end total_time: ")[1].split(" s")[0]
                )
        ...
        if (
            metrics["num_3dgs"] is None
            and "Now num of 3dgs:" in line
            and "Max Memory usage:" in line
            and "Now Pinned Memory:" in line
        ):
            ...
            try:
                metrics["num_3dgs"] = int(
                    line.split("Now num of 3dgs: ")[1].split(".")[0]
                )
                metrics["max_gpu_memory_gb"] = float(
                    line.split("Max Memory usage: ")[1].split(" GB")[0]
                )
                metrics["pinned_cpu_memory_gb"] = float(
                    line.split("Now Pinned Memory: ")[1].split(" GB")[0]
                )
            except:
                pass
    ...
    for subfolder in subfolders:
        ...
        if metrics:
            result_row = {
                ...
                "test_psnr": (
                    round(metrics["test_psnr"], 2)
                    if metrics["test_psnr"] is not None
                    else None
                ),
                "train_psnr": (
                    round(metrics["train_psnr"], 2)
                    if metrics["train_psnr"] is not None
                    else None
                ),
                "num_3dgs": metrics["num_3dgs"],
                "max_gpu_memory_gb": (
                    round(metrics["max_gpu_memory_gb"], 2)
                    if metrics["max_gpu_memory_gb"] is not None
                    else None
                ),
                "pinned_cpu_memory_gb": (
                    round(metrics["pinned_cpu_memory_gb"], 2)
                    if metrics["pinned_cpu_memory_gb"] is not None
                    else None
                ),
                "total_time_s": (
                    round(metrics["total_time_s"], 2)
