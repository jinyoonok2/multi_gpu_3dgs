...
def training(dataset_args, opt_args, pipe_args, args, log_file):
    ...
    if args.enable_distributed:
        ...
        # Auto-scale parameters for multi-GPU training to maintain fair budget
        # When using N GPUs, each sees args.bsz/N samples, but total batch = args.bsz
        # To keep total images seen constant, we scale iterations by 1/world_size
        if args.world_size > 1:
            ...
            if args.rank == 0:
                print(f"\n{'='*60}")
                print(f"Multi-GPU Auto-Scaling Enabled")
                print(f"{'='*60}")
                print(f"World size: {args.world_size} GPUs")
                print(f"Scale factor: {scale_factor:.2f}x")
                print(f"Total batch size: {args.bsz} ({args.bsz // args.world_size} per GPU)")
        ...
        if args.rank == 0:
            print(f"Initialized DDP: {args.world_size} processes")
            print(f"Process {args.rank}: GPU {args.local_rank}")
    else:
    ...
    # Only rank 0 prepares output and logging
    if args.rank == 0:
    ...
    # ------------------------------------------------------------------------
    # 1.3: Initialize data loader with distributed sampler if needed
    # ------------------------------------------------------------------------
    ...
    if args.enable_distributed:
        ...
        # Each GPU processes a fraction of the total batch
        per_gpu_batch_size = args.bsz // args.world_size
        assert args.bsz % args.world_size == 0, f"Batch size {args.bsz} must be divisible by world_size {args.world_size}"
        assert per_gpu_batch_size in [4, 8, 16, 32, 64], f"Per-GPU batch size must be in [4,8,16,32,64], got {per_gpu_batch_size}"
        ...
        if args.rank == 0:
            print(f"Distributed training: {args.world_size} GPUs, total batch_size={args.bsz}, per_gpu_batch_size={per_gpu_batch_size}")
    else:
        per_gpu_batch_size = args.bsz
        sampler = None
        shuffle = True
    ...
    # Only rank 0 should display progress bar to avoid interleaved output
    if args.rank == 0:
    ...
    for iteration in range(
        start_from_this_iteration, opt_args.iterations + 1, iteration_step
    ):
        ...
        # Update progress bar and iteration state (only rank 0)
        if progress_bar is not None:
            if iteration // iteration_step % 30 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}"})
            progress_bar.update(iteration_step)
        ...
        if args.nsys_profile:
            ...
            if (
                ...
                nvtx.range_push(f"iteration[{iteration},{iteration+iteration_step})")

        # Gradually increase spherical harmonics degree (every 1000 iterations)
        if utils.check_update_at_this_iter(iteration, iteration_step, 1000, 0):
        ...
        with torch.no_grad():
            ...
            if (
                ...
                and utils.check_update_at_this_iter(
                    iteration, iteration_step, args.densification_interval, 0
                )
            ):
            ...
            if any(
                [
                    iteration <= save_iteration < iteration + iteration_step
            ...
            if any(
                [
                    iteration <= checkpoint_iteration < iteration + iteration_step
